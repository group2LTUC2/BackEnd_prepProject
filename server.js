...
server.post('/takenitems', postTakenItemsHandler);// inserting taken item into database(takenDonations)
...
function updateDonationHandler(req, res) {//in donation page
  ...
  const sql = `UPDATE donationCard SET quantity=$1 WHERE id=${id} RETURNING *`;
  const value = [donate.quantity];
...
function postTakenItemsHandler(req, res) {
  const cadres = req.body;
  const sql = `INSERT INTO takenDonations (fullName,phonNumber,item,quantity,locationOf,donerName,donerLocation,donerPhone)
